...
!>--- Refinement level
  ...
  if (.not.allocated(calc%temperatures)) then
    if (.not.allocated(env%thermo%temps)) then
      call env%thermo%get_temps()
    end if
    calc%nt = env%thermo%ntemps
    allocate (calc%temperatures(env%calc%nt),source=0.0_wp)

    calc%temperatures = env%thermo%temps
    calc%ithr = env%thermo%ithr
    calc%sthr = env%thermo%sthr
    calc%fscal = env%thermo%fscal
  end if
