...
module hessian_quality
   ...
   type :: step_monitor
      ...
      real(wp) :: gain_threshold = 0.07_wp     !> below this → no new info
...
! ══════════════════════════════════════════════════════════════════════════════
   ...
   subroutine monitor_add_step(mon, s_vec, gain, accepted)
      ...
      mon%call_count = mon%call_count + 1
      accepted = .false.
      gain = 0.0_wp
   ...
   subroutine selective_hessian_repair(mol, calc, modes, eigenvalues, &
      ...
      do i = 1, n_modes
         ...
         !Project into eigenbasis of guess Hessian and replace row in (diagonal) guess Hessian
         !as it is in its eigenbasis

        temp_hess(:,i) = matmul(transpose(modes), Hv)

      end do

      temp_hess = 0.5*(transpose(temp_hess)+temp_hess)
