...
!========================================================================================!
   ...
   subroutine optimize_geometry(mol, molnew, calc, etot, grd, pr, wr, iostatus)
      ...
      if (calc%do_HR .or. calc%deform_opt_hess) then
         ...
         call calc%chess%alloc(mol%nat, calc%hu_steps, calc%initialize_hr_type, calc%hr_hu_type, calc%chess_id_guess)
      end if
      ...
      if (calc%do_HR .and. iostatus .eq. 0) then !> Hessian reconstruction and post-processing happen here, only do it if geometry relaxation successful
        ...
        call initialize_hessian(calc,calc%chess%initialize_type,calc%chess%coords(idx,:,:), &
          &molnew%nat,molnew%at,calc%chess%hess(:),calc%chess%hguess,pr)
